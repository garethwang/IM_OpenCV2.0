...
ImageScale::ImageScale(const Image<float>& I,double r){
  ...
  radius_size=r;
  step=sqrt(2.0);
  int size= std::max(int(I.Width()),int(I.Height()));
...
//#endif
  for (int i=1; i<number; i++){
    ...
    for (size_t j=0;j<I2.Width(); j++){
      for (size_t k=0;k<I2.Height();k++){
        I2(k,j)=static_cast<float>(inter(double(j+0.5)*ratio,double(k+0.5)*ratio,ratio));
      }
    }
...
//#pragma omp parallel for
//#endif
		for (size_t x=1;x<I.Width()-1;x++)
			for (size_t y=1;y<I.Height()-1;y++){
...
float KVLD(const Image<float>& I1,const Image<float>& I2,
	...
	std::vector<Pair>& matchesFiltered,std::vector<double>& score,kvld::matrix<float>& E,std::vector<bool>& valide, KvldParameters& kvldParameters){
		...
		float range1=getRange(I1,std::min(int(F1.size()),int(matches.size())),kvldParameters.inlierRate,kvldParameters.rang_ratio);
		float range2=getRange(I2,std::min(int(F2.size()),int(matches.size())),kvldParameters.inlierRate,kvldParameters.rang_ratio);
					...
					for (size_t it2=it1+1; it2<size;it2++){
						if (valide[it2]){
								...
								&& (dist1<range1 || dist2<range2))
							{
									...
									if(E((int)it1,(int)it2)==-1) //update E if unknow
									{
										E((int)it1, (int)it2) = -2; E((int)it2, (int)it1) = -2;
										...
										if(!kvldParameters.geometry || consistent(F1[a1],F1[a2],F2[b1],F2[b2])<distance_thres)
										{
											...
											if (error<juge)
											{
												E((int)it1, (int)it2) = (float)error;
												E((int)it2, (int)it1) = (float)error;
												//std::cout<<E(it2,it1)<<std::endl;
											}
										}
									}

									if (E((int)it1, (int)it2) >= 0)
									{
										result[it1]+=1;
										result[it2]+=1;
										scoretable[it1] += double(E((int)it1, (int)it2));
										scoretable[it2] += double(E((int)it1, (int)it2));

...
void gaussian_2D_convolution(const cv::Mat& src, cv::Mat& dst, size_t ksize_x,
  ...
  if (sigma > ksize_x || sigma > ksize_y || ksize_x == 0 || ksize_y == 0) {
    ksize_x = (size_t)ceil(2.0*(1.0 + (sigma-0.8)/(0.3)));
    ksize_y = ksize_x;
  }
  ...
  cv::GaussianBlur(src, dst, cv::Size((int)ksize_x, (int)ksize_y), sigma, sigma, cv::BORDER_REPLICATE);
}

/* ************************************************************************* */
void image_derivatives_scharr(const cv::Mat& src, cv::Mat& dst,
                              const size_t xorder, const size_t yorder) {
  cv::Scharr(src, dst, CV_32F, (int)xorder, (int)yorder, 1.0, 0, cv::BORDER_DEFAULT);
}
...
float compute_k_percentile(const cv::Mat& img, float perc, float gscale,
  ...
  for (k = 0; nelements < nthreshold && k < nbins; k++)
    nelements = nelements + (size_t)hist[k];
...
void compute_derivative_kernels(cv::OutputArray kx_, cv::OutputArray ky_,
	...
	const int ksize = int(3 + 2 * (scale - 1));

  // The usual Scharr kernel
  if (scale == 1) {
	  cv::getDerivKernels(kx_, ky_, (int)dx, (int)dy, 0, true, CV_32F);
    return;
  }

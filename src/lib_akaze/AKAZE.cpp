          ...
          if (is_extremum == true) {
            ...
            if (is_out == false) {
              if (is_repeated == false) {
                point.pt.x = point.pt.x*ratio + .5f*(ratio-1.0f);
                point.pt.y = point.pt.y*ratio + .5f*(ratio-1.0f);
...
void AKAZE::Do_Subpixel_Refinement(std::vector<cv::KeyPoint>& kpts) {
  ...
  for (size_t i = 0; i < kpts.size(); i++) {
    ...
    if (fabs(dst(0)) <= 1.0 && fabs(dst(1)) <= 1.0) {
      kpts[i].pt.x = x + dst(0);
      kpts[i].pt.y = y + dst(1);
      int power = (int)powf(2, (float)evolution_[kpts[i].class_id].octave);
      kpts[i].pt.x = kpts[i].pt.x*power + .5f*(power-1);
      kpts[i].pt.y = kpts[i].pt.y*power + .5f*(power-1);
      kpts[i].angle = 0.0;
...
void AKAZE::Compute_Main_Orientation(cv::KeyPoint& kpt) const {
  ...
  for (int i = -6; i <= 6; ++i) {
    for (int j = -6; j <= 6; ++j) {
      if (i*i + j*j < 36) {
        ...
        Ang[idx] = cv::fastAtan2(resY[idx], resX[idx])*(float(CV_PI/180.0));
        ++idx;
      }
    }
  }
  ...
  for (ang1 = 0; ang1 < 2.0*CV_PI;  ang1+=0.15f) {
    ang2 = float(ang1+CV_PI/3.0f > 2.0*CV_PI ? ang1-5.0f*CV_PI/3.0f : ang1+CV_PI/3.0f);
    sumX = sumY = 0.f;
    ...
    if (sumX*sumX + sumY*sumY > max) {
      ...
      kpt.angle =  cv::fastAtan2(sumY, sumX)*float(CV_PI/180.0);
    }
  }
}
    ...
    while (j < pattern_size) {
      ...
      for (int k = i; k < i + 9; ++k) {
        for (int l = j; l < j + 9; ++l) {
          ...
          y1 = fRound(sample_y-.5f);
          x1 = fRound(sample_x-.5f);

          y2 = fRound(sample_y+.5f);
          x2 = fRound(sample_x+.5f);
...
void AKAZE::Get_MLDB_Descriptor_Subset(const cv::KeyPoint& kpt, unsigned char* desc) {
  ...
  steps.at(1) = (int)ceil(2.f*options_.descriptor_pattern_size/3.f);
...
void libAKAZE::generateDescriptorSubsample(cv::Mat& sampleList, cv::Mat& comparisons, int nbits,
  ...
  for (int i=0, c=0; i<3; i++) {
    ...
    int gsz = gdiv*gdiv;
    int psz = (int)ceil(2.0f*pattern_size/(float)gdiv);
  ...
  srand(1024);
  cv:: Mat_<int> comps = cv::Mat_<int>(int(nchannels*ceil(nbits/(float)nchannels)),2);
  comps = 1000;
